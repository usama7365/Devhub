    ...
    <div className="space-y-24 py-12">
      ...
      <section className="text-center">
        ...
        <p className="text-xl text-gray-600 dark:text-gray-400 max-w-3xl mx-auto mb-8">
          Join our thriving community of developers to collaborate, share knowledge, and grow together.
        </p>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
            ...
            <div className="space-y-4">
              {dummyPosts.slice(0, 3).map(post => (
                <Link key={post.id} to={`/discussions/${post.id}`} className="block hover:bg-gray-50 dark:hover:bg-gray-700 -mx-6 px-6 py-3">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
            ...
            <div className="space-y-4">
              {dummyArticles.slice(0, 3).map(article => (
                <Link key={article.id} to={`/knowledge-base/${article.id}`} className="block hover:bg-gray-50 dark:hover:bg-gray-700 -mx-6 px-6 py-3">
                  <h4 className="font-medium text-gray-900 dark:text-gray-100">{article.title}</h4>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{article.description}</p>
                </Link>
              ))}
            </div>
          </div>
      ...
      <section className="bg-indigo-600 dark:bg-indigo-700 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 py-16 text-center">
        ...
        <p className="text-indigo-100 mb-8 max-w-2xl mx-auto">
          Start collaborating with developers from around the world. Join our community today!
        </p>
        <Link to="/signup" className="inline-flex items-center px-6 py-3 border border-transparent text-lg font-medium rounded-md text-indigo-600 bg-white hover:bg-indigo-50">
          Get Started
        </Link>
      </section>
    </div>
...
function StatCard({ number, label }: { number: string; label: string }) {
  return (
    <div className="text-center">
      <div className="text-4xl font-bold text-indigo-600 dark:text-indigo-400">{number}</div>
